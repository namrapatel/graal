...
contract PlayerJoinSystem is System {
  ...
  function requirement(bytes memory arguments) public view returns (bytes memory) {
    ...
    return abi.encode(playerComponent, playerEntity); 
  }

  function execute(bytes memory arguments) public returns (bytes memory) {
    (PlayerComponent playerComponent, uint256 playerEntity) = abi.decode(requirement(arguments), (PlayerComponent, uint256));

...
uint256 constant ID = uint256(keccak256("graal.prototype.CastleFort"));

function CastleFortPrototype(IUint256Component components) {
  ...
  RoomComponent(getAddressById(components, RoomComponentID)).set(ID, uint32(Room.Castle));
  RoomTypeComponent(getAddressById(components, RoomTypeComponentID)).set(ID, uint32(RoomType.Fort));
  LocationComponent(getAddressById(components, LocationComponentID)).set(ID, uint32(Room.Castle));
